...
def howto_txt_content() -> str:
    return (
        "1) Upload location\n"
        "- Unzip into public_html or a subfolder.\n\n"
        "2) PHP version\n"
        "- Works with 7.4–8.2; recommend 8.1/8.2.\n\n"
        "3) Subdomain\n"
        ...
        "6) Endpoints\n"
        "- Upload: POST /api/blob/upload.php (form-data: file, optional folder).\n"
        "- Delete: POST /api/blob/delete.php (JSON: {path}).\n"
        "- List: GET /api/blob/list.php (folder, page, per_page).\n"
        "- Ping: GET /api/blob/ping.php.\n"
        "- Auth: send X-Halal-Blob-Key header (or ?key= fallback).\n\n"
        "7) Meta\n"
        "- Metadata JSON is stored under meta/ mirroring blob folders.\n\n"
        "8) Builder\n"
        "- Run with: uv run build_halal_custom_blob_setup.py\n"
        "- Fallback: python build_halal_custom_blob_setup.py\n\n"
        "9) SDK\n"
        "- A Node/Next.js SDK file (sdk/node/halalBlobClient.ts) is generated next to the builder.\n"
        "- It is NOT part of the ZIP.\n"
    )
